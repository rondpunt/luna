...
function returnNudge(checkIns) {
  if (!checkIns?.length) return "Een rustig moment om in te checken bij jezelf.";
  ...
  if (daysSince > 14) return "Welkom terug. Fijn dat je er bent.";
  const thisWeek = checkIns.filter((c) => differenceInDays(new Date(), parseISO(c.created_date)) < 7).length;
  if (thisWeek >= 7) return "Elke dag aanwezig — knap.";
  if (thisWeek >= 3) return "Mooi bezig deze week.";
    ...
    <div className="fade-in" style={{
      ...
      <header>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          <JunieLogo variant="stacked" size={32} />
          {streak > 1 && (
            <motion.div
              ...
              style={{
                ...
                boxShadow: "0 4px 12px rgba(240, 146, 94, 0.18)",
              }}
            >
              <Flame size={13} style={{ color: "#F0925E" }} strokeWidth={2.4} />
              <span style={{ fontSize: 13, fontWeight: 700, color: "#D67541", letterSpacing: "-0.01em" }}>{streak}</span>
            </motion.div>
          )}
        </div>

        <p className="eyebrow-muted" style={{ marginBottom: 6 }}>{format(new Date(), "EEEE d MMMM", { locale: nl }).toUpperCase()}</p>
        ...
        <motion.h1
          ...
          style={{ fontSize: 34, color: "var(--text)", letterSpacing: "-0.02em", lineHeight: 1.1, marginTop: 4 }}
        >
          {greeting}{firstName ? `,` : "!"}
          {firstName && (
            <span style={{
              ...
              backgroundClip: "text", display: "inline-block", marginLeft: 8,
            }}>{firstName} ✨</span>
          )}
        </motion.h1>
        <motion.p
          ...
          style={{ fontSize: 15, color: "var(--text-soft)", marginTop: 8, lineHeight: 1.45 }}
        >
          {nudge}
        </motion.p>
      </header>
      ...
      <section>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14 }}>
          <h2 className="font-display-bold" style={{ fontSize: 20, color: "var(--text)", letterSpacing: "-0.02em" }}>
